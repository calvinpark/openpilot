...
class FullScreenApp(QWidget):
    ...
    def read_persist_key(self):
        ...
        if key_path.exists():
            ...
            if len(raw_key) == MAX_INPUT_LENGTH and all(c in HEX_ALPHABET for c in raw_key.lower()):
                return raw_key
        return ""
    ...
    def init_ui(self, default_text):
        ...
        self.hex_buttons_row1, self.hex_buttons_row2 = self.create_hex_buttons()
        self.setup_layout()
        self.colors = DARK_COLORS
        self.update_text_color()

    def create_hex_buttons(self):
        row1_chars = [str(x) for x in range(10)]
        row2_chars = list("abcdef") + [BACKSPACE_CHAR]
        row1, row2 = [], []

        for ch in row1_chars:
            btn = QPushButton(ch)
    ...
    def on_button_clicked(self, char):
        ...
        if len(current_text) < MAX_INPUT_LENGTH:
            self.insert_colored_text(char, len(current_text))

    def insert_colored_text(self, char, position):
        cursor = self.input_box.textCursor()
        fmt = QTextCharFormat()
        color_index = (position // 4) % len(self.colors)
        fmt.setForeground(QColor(self.colors[color_index]))
    ...
    def update_text_color(self):
        ...
        if len(text) == MAX_INPUT_LENGTH:
            ...
            self.full_input = False
            remaining = MAX_INPUT_LENGTH - len(text)
            self.input_label.setText(f"{remaining} characters remaining")
    ...
    def save_key(self):
        ...
        if len(key) == MAX_INPUT_LENGTH:
            try:
                p1 = Path("/data/params/d/SecOCKey")
                p2 = Path("/persist/tsk/key")
                for p in [p1, p2]:
    ...
    def toggle_interface(self, visible):
        ...
        if self.full_input:
            self.input_label.setVisible(False)
            self.save_button.setVisible(visible and not self.overlay_visible)
        else:
