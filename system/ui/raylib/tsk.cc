...
namespace {
  ...
  const Color kGray = {96, 96, 96, 255};
  ...
  const std::string kRunButtonText = "Extract key\nfrom the car";
  const std::string kKeyboardButtonText = "Manual Install";
  const std::string kExitText = "Exit";
  const std::string kRunScriptPath = kBasePath + "/tsk/tsk-ext.sh";
...
int main(int argc, char* argv[]) {
  ...
  while (!WindowShouldClose()) {
    ...
    // Run Button
    DrawButton(runButtonRect, kRunButtonText, regularFont, kGray);
    if (CheckCollisionPointRec(GetMousePosition(), runButtonRect) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
      system(kRunScriptPath.c_str());
    }
